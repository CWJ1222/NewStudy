...
public class 배열 {
	...
	private static void f4() {
		...
		// 다차원 배열
		// 배열참조 변수 선
		int[][] arr; // 자바는 타입에 사용하는 것을 많이 함
		int[] arr2[];
		int arr3[][];

		// 배열객체생성
		arr = new int[3][4]; // 3행4열, arr의 length는 3, 각 하나의 length가 4, arr은 3개짜리 배열을 참조
		arr2 = new int[3][]; // null이 3개에 차 있다.
		// arr3 = new int[][]; // 첫번째 대괄호는 필수로 넣어야 함
		arr3 = new int[][] { { 1, 2, 3 }, { 10, 20 } }; // 첫번째 대괄호는 필수로 넣어야 함, 행은2개인데 열을 다르다.

		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr.length; j++) {
				System.out.printf("%d행 %d열 %d값\t", i, j, arr[i][j]);
			}
			System.out.println();
			System.out.println();
		}
		...
		for (int i = 0; i < arr2.length; i++) {
			arr2[i] = new int[5 - i]; // 이걸 써줘야 에러가 안남.
	...
	private static void f3() {
		...
		System.out.println(scores[5]);// 없음
	}
